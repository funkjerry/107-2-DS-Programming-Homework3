...
```{r ans11}
# loss1Companies
loss1Companies <- (finStatement$`營業利益（損失）`<0)
...
```{r ans12}
# loss2Companies
loss2Companies <- (finStatement$`基本每股盈餘（元）`<0)
...
```{r ans13}
# only1LossCompanies
only1LossCompanies <- (loss1Companies ="TRUE" & loss2Companies ="FALSE") or (loss1Companies ="FALSE" & loss2Companies ="TRUE")
```
...
```{r ans22}
# df_oneShow
df_oneshow <- data.frame(exhibitionList[1]c(1,2,3,4,5,6,7),stringsAsFactors = F)
df_oneshow
```
